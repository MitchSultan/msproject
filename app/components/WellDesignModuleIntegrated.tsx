...
} from 'lucide-react'; // Assuming the file is in the same directory
...
const DEFAULT_PROPS: Record<'seawater' | 'formation', LayerProperties> = {
  seawater: {
    density: 8.5, viscosity: 1.0, thermal_conductivity: 0.6,
    heat_capacity: 4184, pore_pressure: 8.5, fracture_gradient: 9.0
  },
  formation: {
    density: 11.0, viscosity: 15.0, thermal_conductivity: 2.5,
    heat_capacity: 850, pore_pressure: 10.5, fracture_gradient: 13.5
  }
};

const WellDesignModuleIntegrated: React.FC = () => {
  ...
  const [layers, setLayers] = useState<LithologyLayer[]>([
    {
      id: '1', name: 'Seawater Layer', type: 'seawater',
      topDepth: 0, bottomDepth: 3000, geothermalGradient: 0.5,
      properties: { ...DEFAULT_PROPS.seawater }
    },
    {
      id: '2', name: 'Main Formation', type: 'formation',
      topDepth: 3000, bottomDepth: 12000, geothermalGradient: 1.8,
    ...
    <div className="min-h-screen bg-slate-950 text-slate-200 p-8 font-sans">
      ...
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="text-slate-400 mt-2">Configure geological layers and visualize the well profile in real-time.</p>
        </div>
      ...
      <div className="grid grid-cols-12 gap-8">
        {/* Left Sidebar: Environment Settings */}
            ...
            <div className="space-y-4">
              ...
              {environment.location === 'offshore' && (
                <div>
                  ...
                  <input 
                    type="number" 
                    className="w-full bg-slate-950 border border-slate-800 rounded-md px-3 py-2 text-sm focus:border-blue-500 outline-none font-mono"
                    value={environment.waterDepth}
                    onChange={(e) => {
                      const newDepth = Number(e.target.value);
                      setEnvironment({...environment, waterDepth: newDepth});
                      // Sync first layer depth if it's seawater
                      if (layers[0]?.type === 'seawater') {
                        updateLayer(layers[0].id, 'bottomDepth', newDepth);
                        if (layers[1]) updateLayer(layers[1].id, 'topDepth', newDepth);
                      }
                    }}
                  />
                </div>
        ...
        <div className="col-span-12 lg:col-span-9">
                  ...
                  {layers.map((layer) => (
                    <tr key={layer.id} className="hover:bg-slate-800/30 transition-colors group">
                      ...
                      <td className="px-6 py-4">
                        <input 
                          type="number"
                          className="bg-transparent border-none focus:ring-0 text-sm font-mono text-slate-300 w-24 outline-none"
                      ...
                      <td className="px-6 py-4">
                        <input 
                          ...
                          onChange={(e) => {
                            ...
                            updateLayer(layer.id, 'bottomDepth', newBottom);
                            // Auto-sync next layer's top depth
                            const nextIndex = layers.findIndex(l => l.id === layer.id) + 1;
                            if (layers[nextIndex]) {
                              updateLayer(layers[nextIndex].id, 'topDepth', newBottom);
                            }
                          }}
                        />
                      </td>
          ...
          <div className="mt-6 p-4 bg-blue-500/5 border border-blue-500/10 rounded-xl flex items-start gap-4">
      ...
      {/* Property Editor Modal (Reused from previous version) */}
      {editingLayerId && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-slate-900 border border-slate-800 rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl">
            ...
            <div className="p-6 bg-slate-950/50 border-t border-slate-800 flex justify-end">
              <button onClick={() => setEditingLayerId(null)} className="bg-blue-600 hover:bg-blue-700 text-white px-8 py-2 rounded-xl font-bold transition-all">
                Done
              </button>
            </div>
          </div>
...
export default WellDesignModuleIntegrated;